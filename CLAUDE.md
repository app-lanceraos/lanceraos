...
**Design system (`InvoiceDesign.design_data`) — backend contract, editor, real render path, AND real
design-to-invoice assignment all built (Steps 8/8b, render path + assignment fix both 19 August
2026)**: a saved design now genuinely affects real invoice PDF/portal output end to end — a saved
design's `design_data` is actually rendered (`apps/invoices/design_renderer.py`, closed audit finding
PDF-001), AND a real invoice actually gets that design assigned in the first place (`invoice_create`/
`_finalise_invoice` reading the user's real default design — closed a real, live-browser-verified SEV1
report the same day; see this module's own two 19 August 2026 entries above and DECISIONS.md for the
full design of each). Before the second fix, the first fix's own renderer had zero real-world effect —
`Invoice.design` was never assigned by anything, so no real invoice ever reached the dynamic path
regardless of what any user built. The backend contract itself is a documented two-zone JSON schema

...
- ~~GET/POST /api/invoices/designs/ + GET/PUT/DELETE /api/invoices/designs/{id}/~~ /
  ~~POST /api/invoices/designs/{id}/set-default/ + /designs/duplicate/~~ / ~~POST
  /api/invoices/designs/ai-seed/~~ — **all REMOVED, Post-Reversion Polish, 12 September 2026** (the
  whole `designs/*` route group and `InvoiceDesign` itself are gone; "Use this template" is a
  direct `PUT /api/auth/profile/` — see this module's own 12 September 2026 entries and
  DECISIONS.md). Struck through here rather than deleted from this list, since the paragraphs
  immediately below still describe the Step 8/8b/9 system these endpoints belonged to as real
  history.
- GET /api/invoices/templates/ (**13 September 2026, Template Gallery Foundation** — the real
  replacement read surface: a static, `IsAuthenticated`, read-only catalog of selectable templates
  from `apps.invoices.template_manifest`, explicitly not a per-user-row endpoint like the one it
  replaces)
...
**13 September 2026 (Template Gallery Foundation, Pass 1 of N on the 20-template import).** Builds
the foundation the 20 incoming templates will depend on, proven against the 3 that already exist —
adds no new template itself. `apps/invoices/template_manifest.py` (`TEMPLATES`) is now the one
authoritative list of invoice templates, replacing 3 previously-independent copies
(`FreelancerProfile.INVOICE_TEMPLATE_CHOICES` stays a deliberate mirror tuple, guarded by a real
drift test, since `apps.users` cannot import `apps.invoices`; `Invoice.base_template`'s own choices
are now a direct import, same app). A new `GET /api/invoices/templates/` read-only endpoint serves
`selectable_templates()` to the renamed gallery page (`DesignGallery.jsx` → `TemplateGallery.jsx`,
route `/invoices/designs` → `/invoices/templates` with a real redirect kept at the old path). A new
`FreelancerProfile.show_lanceraos_branding` toggle (default `True`, UI on the Template Gallery page
only) is now the sole input to `pdf_generator._is_premium_branding_enabled` — the one existing hook
every render path already read through, so no template needed to change to gate on it. The footer
across all 3 static templates was rewritten to a real 4-cell rule (business info + wordmark on a
single page with branding on; NO footer at all on a single page with branding off; business info +
page counter, with or without the wordmark, on any multi-page render) and extracted to this
codebase's first shared template partial, `apps/invoices/templates/invoices/_partials/footer.html`
— real WeasyPrint capability confirmed first via an isolated spike (two-line margin-box content,
page-count parity with the footer margin removed, and 3 real 16mm-margin-derived arithmetic
dependencies found and fixed, one more than expected: `minimal.html`'s own `body.body-single`
min-height, not just `professional.html`'s `.page` and `modern.html`'s `.sidebar`). A genuine,
unrelated dead-file finding surfaced during this pass's own file inventory and is flagged, not
fixed: 5 template files from the removed free-canvas system
(`apps/invoices/templates/invoices/canonical/` and 3 loose `dynamic_*`/`_dynamic_*.html` files)
were left behind by the 12 September 2026 Full Reversion's own sweep — confirmed zero references
...
**14-15 September 2026 (20-Template Import, Batches 2-5 — COMPLETE).** The remaining 4 batches
landed the rest of the 20-template import: Batch 2 (`free_compact`, `free_freelancer` + legacy
Minimal retired from the gallery — a same-named, structurally unrelated free-tier Minimal replaces
it in Batch 3); Batch 3 (`free_minimal`, `free_modern`, `free_professional`, `free_business`);
Batch 4 (`pro_executive`, `pro_studio`, `pro_signature`, `pro_editorial`, `pro_grid`,
`pro_commerce`, `pro_prestige` — the first 7 pro-tier templates, plus 2 new shared
`parties_row.html`/`meta_strip.html` parameters, `from_label`/`bill_to_label`/`from_only`/
`with_invoice_number`, covering real custom-party-label and header-identity deviations); Batch 5,
FINAL (`pro_atelier`, `pro_consulting`, `pro_statement` — the 3 persistent-rail/sidebar designs,
ported via the same `position: fixed` technique modern.html's own sidebar and free_minimal.html's
own rail already proved, repeating on every real page automatically rather than the prototype's
own page-1-only limitation). The gallery is now complete: **22 real, selectable templates (11
free, 11 pro)** out of 23 total manifest rows (legacy Minimal remains present but retired,
`selectable: False`, since real historical `Invoice.base_template='minimal'` rows must keep
rendering forever). Every selectable template now has a real, pre-generated gallery preview PNG
(`generate_template_previews.py` — Batch 5 also closed a real, accumulated gap here: this command
was never updated across Batches 1-4, so 16 of 19 templates were showing a broken-image icon in
the real gallery UI until Batch 5's own Part 3 fixed it, once, for the complete set, by reading the
template list from the manifest instead of a hardcoded 3-template tuple). Two real, confirmed
WeasyPrint 69.0 platform limitations were found and worked around during Batch 5 (`writing-mode`
and `filter` are both silently-ignored "unknown property" cases — no vertical text, no grayscale
filter effect are achievable in this renderer). Backend test growth across the whole import: 974 →
1086 (`python manage.py test`). See DECISIONS.md's 14-15 September 2026 entries (one per batch,
plus Batch 5's own closing summary) for the full per-batch reasoning, every real per-template
...
hyperlink).** Two more shared-partial bugs closed across all 23 templates: `_partials/footer.html`
gained a calibrated `footer_right_inset` (mirroring `footer_left_inset`) so the "Generated by
[wordmark]" content lands at the same real absolute inset from the right edge that the business-info
text already has from the left — previously unset, so the real gap varied by up to ~22mm across
templates. `_partials/pay_online.html` and its 3 hand-rolled equivalents (`modern.html`,
`pro_signature.html`, `pro_statement.html`) are now real `<a href="{{ invoice.payment_page_url }}">`
links, not plain `<div>`s — the QR block was previously unclickable in every rendered PDF. See
DECISIONS.md's 18 September 2026 entry for the full per-template measurements, the real WeasyPrint
margin-box quirks found along the way, and the full verification evidence.

**CORRECTION, same day.** The `footer_right_inset` values above were calibrated to the wrong
element — the "Generated by " text's position, not the trailing wordmark's own real vector bounding
box, which the original measurement script couldn't see at all (`get_text()`/`get_image_info()`
both miss an SVG path fill). This shipped a real ~1.4-5.4mm wordmark overflow past the true page
edge on all 23 templates. Recalibrated the same day using a `page.get_drawings()`-based measurement
proven against the known-broken state first, with a deliberate +1.0mm safety margin added on top of
the 16mm/12mm targets (landing at 17mm/13mm) specifically because the original "exact to
sub-0.001mm" claim is what let this ship undetected. See DECISIONS.md's second 18 September 2026

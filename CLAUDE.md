...
**Design system (`InvoiceDesign.design_data`) — backend contract, editor, AND the real render path
all built (Steps 8/8b, render path 19 August 2026)**: a saved design now genuinely affects real
invoice PDF/portal output (`apps/invoices/design_renderer.py`, see this module's own 19 August 2026
"design_data render path" entry above and DECISIONS.md for the full design) — previously validated
and stored but never read by any render path (closed audit finding PDF-001). The backend contract
itself is a documented two-zone JSON schema (`apps/invoices/design_schema.py`) — `zone_1` (logo/business_info/

...
{
  "schema_version": 2,
  "page": { "size": "A4", "width_mm": ..., "height_mm": ..., "margin_*_mm": ..., "sidebar": <bool>, "footer": <object, optional — see "Phase 1 schema additions" below> },
  "header": {
    "elements": [
      {
        "kind": "semantic" | "generic",
        "type": "logo" | "business_info" | "client_info" | "dates" | "text" | "image" | "rectangle" | "divider" | "container" | "ellipse",
        ...
        "rotation": <degrees, optional, default 0, "pinned" elements only>,
        "crop": <{x,y,width,height} fractions 0-1, optional, generic "image" only>,
...
### Phase 1 schema additions (rotation / ellipse / page.footer / crop, 07 September 2026)

Four additive extensions to the production (schema_version 2) shape above, added ahead of the
standalone template-builder editor replacing `DesignEditor.jsx` (GrapesJS) — the canonical
renderer, validation, versioning, gallery, and invoice-assignment pipeline are all unchanged;
these are pure schema/renderer additions. Every existing row (v2 or legacy) is unaffected: none of
the four new keys are ever written by anything that predates this phase, so absence means "exactly
today's behavior" in all four cases.

- **`rotation`** (optional float, degrees, any element in `header` or `flow`; absent means `0`).
  Rendered as a real `transform:rotate(Ndeg)` (`design_renderer.prepare_element`). Restricted to
  `layout_mode: "pinned"` elements (the default) — rejected at validation with a clear message on
  a `layout_mode: "flow"` element, since a flow element's real footprint is content-driven and
  unknown at design time. `design_schema._validate_overlap` and `_validate_page_bounds` both now
  compare each element's REAL rotated footprint (`design_schema.rotated_bounding_box` — a direct
  Python port of the editor's own `utils/geometry.js` `rotatedBoundingBox`), not its raw
  unrotated box, so a rotated element that visibly overlaps a neighbor (or pokes off the page) is
  caught even when its own unrotated x/y/width/height looks safe, and — symmetrically — a rotated
  element whose unrotated box looks like it collides with a neighbor but whose real footprint
  (e.g. a long thin box rotated close to 90°) doesn't, is correctly NOT flagged. Byte-identical to
  the pre-Phase-1 behavior for every element with rotation absent or `0`.
- **`ellipse`** — a 6th `GENERIC_TYPES` entry. Renders through the exact same fill/border
  resolution `rectangle`/`container` already use (`design_renderer.attach_generic_content`), plus
  one forced `border-radius:50%` (never user-configurable — an "ellipse" that isn't elliptical
  would just be a confusingly-named rectangle). A real, live-rendered bug was found and fixed
  while adding this: the outer element wrapper (`.v2-el`) was ALSO painting a plain, always-sharp-
  cornered `background-color` from the same `style.background_color` key the shape's own fill
  resolves from — invisible for a rectangle (both boxes identical), but it silently hid an
  ellipse's own rounding behind an opaque rectangle in the same color underneath it. Fixed by
  excluding `rectangle`/`container`/`ellipse` (`design_renderer.SHAPE_TYPES_WITH_OWN_FILL`) from
  the generic per-element wrapper background-color — those 3 types manage their own fill
  authoritatively via `shape_css`.
- **`page.footer`** (optional object; absent means no footer at all, exactly today's behavior) —
  a config object, not a positioned canvas element: it has no x/y/width/height of its own, and
  turns on the SAME fixed 3-slot footer content the 3 static templates already show (business
  name + owner email left; a real, genuinely multi-page-only "Page X of N" counter center;
  "Generated by" + the LanceraOS wordmark right), never a per-slot rebindable layout. `style`
  (all optional, defaulted by the renderer — `design_renderer.FOOTER_STYLE_DEFAULTS`) covers
  `text_color`/`background_color`/`divider_color`/`font_family`/`font_size_pt`/`font_weight` and a
  forward-compatible `show_wordmark` flag (combined at render time with the one real, existing
  `_is_premium_branding_enabled` hook — `pdf_generator.py` — not a second, parallel premium
  concept). Rendered as real WeasyPrint `@page` margin boxes (`@bottom-left`/`@bottom-center`/
  `@bottom-right`), the identical real, working mechanism `professional.html`/`minimal.html`/
  `modern.html` already use — reserving a real, additional `@page` bottom margin
  (`design_renderer.FOOTER_MARGIN_BOX_HEIGHT_MM`, 16mm, matching those templates' own value)
  ONLY when a footer is configured. Because WeasyPrint has no `@page :first:last` ("exactly one
  page") CSS selector, whether to even show the counter can only be decided from a REAL rendered
  page count — `design_renderer.render_design_pdf_bytes` now carries the identical MANDATORY SAFE
  ORDER `pdf_generator.render_invoice_pdf`'s own docstring already established for the static
  templates' identical problem: render once assuming multi-page, read the real page count off
  WeasyPrint's own `Document.pages`, and only when that count is exactly 1, re-render once more
  with the counter omitted. `pdf_generator.render_invoice_pdf`'s own v2 branch now calls this
  same function instead of a bare single-shot render, so real invoice PDFs get this for free; a
  v2 design with no footer configured skips the whole mechanism (single render, unchanged
  performance).
- **`crop`** (optional object on a generic `image` element only: `{x, y, width, height}`, each a
  fraction 0-1 of the SOURCE image's own dimensions; absent means the full, uncropped image).
  Non-destructive by construction — the original asset (`style.src`) is never re-encoded or
  replaced; the crop rectangle is stored as data alongside the element's own geometry, so
  re-cropping never compounds quality loss. Rendered via the standard CSS crop-without-re-
  encoding technique (a sized `overflow:hidden` wrapper around a `position:absolute`,
  percentage-scaled-and-shifted `<img>` — `design_renderer.attach_generic_content`'s
  `crop_css`) — confirmed directly against a real WeasyPrint render, not assumed, that this
  technique renders correctly (a 2x2-quadrant test image cropped to exactly one quadrant, scaled
  to fill its box, with zero distortion of the crop itself).

Full verification (real WeasyPrint renders, real page-count assertions across single- and

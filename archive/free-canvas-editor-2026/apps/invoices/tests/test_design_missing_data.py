...
class MissingFlowContentTests(TestCase):
    ...
    def test_signature_block_omitted_entirely_when_no_signature_image_is_set(self):
        # profile.signature_url is blank by default (not set in this
        # class's own setUp) — neither the image nor the "Authorised
        # signature" line/label means anything without a real signature,
        # so the whole block must disappear, not just the image (unlike
        # logo/payment_info/etc., "signature" used to be treated as
        # always-real content; see design_renderer._element_has_real_content).
        # Checked both-case, matching test_design_pagination.py's own
        # established precedent for this exact letter-spaced label (real
        # WeasyPrint/PyMuPDF text extraction can tokenize it either way).
        self.assertEqual(self.user.profile.signature_url, '')
        positions, _ = self._render()
        self.assertIsNone(find(positions, 'Authorised signature'))
        self.assertIsNone(find(positions, 'AUTHORISED SIGNATURE'))

    def test_signature_block_renders_when_a_real_signature_image_is_set(self):
        self.user.profile.signature_url = REAL_LOGO_DATA_URI
        self.user.profile.save()
        positions, _ = self._render()
        found = find(positions, 'Authorised signature') or find(positions, 'AUTHORISED SIGNATURE')
...
class LayersPanelHideFlagTests(TestCase):
    ...
    def test_hiding_does_not_affect_content_mode_alias_the_editor_canvas_still_shows_everything(self):
        # design_canvas.py (the old GrapesJS editor's own canvas adapter,
        # removed along with that editor — see DECISIONS.md's removal
        # entry) never called _element_has_real_content at all (confirmed
        # by source inspection, not just behavior, while it existed) — a
        # hidden element had to stay fully editable/visible in that
        # canvas, only excluded from the CANONICAL renderer's real/alias-
        # independent output. Verified here directly against
        # _element_has_real_content itself with content_mode='alias', the
        # mode that canvas always used (now unused generally — see
        # design_renderer.py's own ALIAS_BINDING_LABELS comment).
        ...
        # Still False in alias mode too — this function's own contract
        # (once hidden, never "has real content" for the CANONICAL
        # renderer regardless of mode) stays internally consistent either
        # way, independent of whether anything currently calls alias mode.

...
class BindingResolutionTests(RendererDatabaseTestCase):
    def test_all_approved_bindings_are_implemented(self):
        ...
        # docstring). Phase 3a (07 September 2026, editor capability
        # parity) grew it again to 28 — `invoice.client_currency_conversion`
        # and `invoice.tax_rate`, both real, existing Invoice surface with
        # no prior binding (see design_schema.SUPPORTED_BINDINGS' own
        # comment). The parity assertion (never allow BINDING_RESOLVERS
        ...
        self.assertEqual(len(SUPPORTED_BINDINGS), 28)
    ...
    def test_generic_text_element_with_binding_resolves_in_full_render(self):
        ...
        # Phase 5.1: y=70 is real, deliberately-checked empty space in the
        ...
        # Moved again, 68->70, 30 August 2026 — the seed's own real new
        # `<hr class="rule">` divider element now occupies y=68-69, see
        # design_templates.py's own `_divider` fidelity fix.)
        design['header']['elements'][0] = {
            'kind': 'generic', 'type': 'text', 'x': 20, 'y': 70, 'width': 50, 'height': 5,
...
class GeometryTests(RendererDatabaseTestCase):
    ...
    def test_no_px_conversion_math_appears_anywhere_in_the_renderer_module(self):
        ...
        # concern (both editors' own units.js/constants.js boundary
        # conversion — never this module's) that has no business here.
        # Checks for the actual conversion-math anti-patterns (this
        # codebase's own real mm<->px constants/helpers, historically
        # frontend/src/lib/designEditor/constants.js/serialization.js,
        # now frontend/src/pages/design-editor-v2/utils/units.js),
...
class GoldenStructureTests(RendererDatabaseTestCase):
    def test_migrated_professional_seed_contains_every_expected_structural_piece(self):
        # Real data: URI, same established precedent as this file's own
        # base fixture (RendererDatabaseTestCase) — required as of the
        # signature-block-collapses-when-unset fix
        # (design_renderer._element_has_real_content): the "Authorised
        # signature" label no longer renders without a real signature_url.
        self.context['freelancer'].signature_url = (
            'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII='
        )
        html = render_design_html(_v2('professional'), self.context)
        self.assertIn(self.context['invoice'].invoice_number, html)

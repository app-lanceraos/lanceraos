...
class GoldenReferenceStructuralParityTests(TestCase):
    ...
    def setUp(self):
        ...
        # Set for the same reason as `logo` above (a real data: URI, no
        # network dependency) — since the signature block now collapses
        # entirely with no real signature_url (see design_renderer.py's
        # _element_has_real_content and the 3 static templates' own
        # `{% if signature_url %}` guard), leaving this unset would make
        # test_professional_signature_label_position's own "AUTHORISED
        # SIGNATURE" needle genuinely absent from BOTH renderers, not a
        # real position to compare — this class's "everything filled in"
        # reference intent already implied a real signature was meant to
        # be present here.
        self.freelancer.signature_url = (
            'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII='
        )

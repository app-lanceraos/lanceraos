...
class V2SchemaValidationTests(SimpleTestCase):
    ...
    def test_flow_element_overlap_detected_now_that_flow_is_free_form(self):
        ...
        # design_schema.py's own docstring. Uses the table (structural,
        # real content) against totals (semantic, real content) rather
        # than the divider used previously — a decorative generic type
        # like divider is now exempt from this check entirely (see
        # OVERLAP_EXEMPT_GENERIC_TYPES / test_decorative_generic_overlap_
        # exempt_from_overlap_check below), so it's no longer a valid
        # fixture for "real content overlap is still rejected".
        d = copy.deepcopy(VALID_V2_DESIGN)
        table = next(e for e in d['flow']['elements'] if e['type'] == 'table')
        table['x'] = 112  # table now overlaps totals (x=112..174, y=130..165)
        table['y'] = 150
        errors = validate_design_data_schema_v2(d)
        self.assertTrue(any('overlaps' in e for e in errors))

    def test_decorative_generic_overlap_exempt_from_overlap_check(self):
        # Matches the editor's own checkOverlaps (frontend/src/pages/
        # design-editor-v2/utils/validation.js): it only ever builds its
        # pairwise overlap list from `item.kind === 'content'` — a shape
        # (rectangle/divider/container/ellipse) or an image is never
        # compared against anything, content or another shape/image
        # ("Shapes and images are collision-exempt by design", that
        # file's own comment). This constructs a payload the frontend
        # already allows (a decorative divider fully overlapping the
        # totals block) and confirms the backend now agrees — client and
        # server overlap rules no longer disagree on this case.
        d = copy.deepcopy(VALID_V2_DESIGN)
        divider = next(e for e in d['flow']['elements'] if e['type'] == 'divider')
        divider['x'] = 112  # divider now fully overlaps totals (x=112..174, y=130..165)
        divider['y'] = 150
        divider['width'] = 62  # shrink so it stays within the page's own right edge
        errors = validate_design_data_schema_v2(d)
        self.assertEqual(errors, [], f'decorative divider-vs-content overlap should be exempt, got: {errors}')
    ...
    def test_decorative_generic_image_also_exempt_from_overlap_check(self):
        # The editor's own `kind: 'image'` items (logo/signature uploads
        # placed as free-form generic images, distinct from the semantic
        # `logo` element) are collision-exempt in checkOverlaps exactly
        # like shapes — confirmed by reading validation.js's own
        # `contentItems` filter (`kind === 'content'` only) and its
        # comment ("Shapes and images are collision-exempt by design").
        # A generic `image` element fully overlapping totals must pass.
        d = copy.deepcopy(VALID_V2_DESIGN)
        d['flow']['elements'].append({
            'kind': 'generic', 'type': 'image', 'x': 112, 'y': 130, 'width': 62, 'height': 35,
            'style': {}, 'overrides': {},
        })
        errors = validate_design_data_schema_v2(d)
        self.assertEqual(errors, [], f'decorative image-vs-content overlap should be exempt, got: {errors}')

    def test_generic_text_still_checked_for_overlap_unlike_shapes_and_images(self):
        # A generic `text` element maps to the editor's `kind: 'content'`
        # (real bound-or-custom text, not decoration) — it must remain a
        # real content-vs-content check, never accidentally swept into
        # the shape/image exemption above. Two generic-text elements
        # placed on top of each other in flow must still be rejected.
        d = copy.deepcopy(VALID_V2_DESIGN)
        d['flow']['elements'].append({
            'kind': 'generic', 'type': 'text', 'x': 112, 'y': 130, 'width': 62, 'height': 35,
            'style': {}, 'overrides': {}, 'binding': 'invoice.number',
        })
        errors = validate_design_data_schema_v2(d)
        self.assertTrue(any('overlaps' in e for e in errors))

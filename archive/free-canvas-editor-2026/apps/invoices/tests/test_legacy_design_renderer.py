# apps/invoices/tests/test_legacy_design_renderer.py
"""
Production cutover — this file now covers `legacy_design_renderer.py`,
the read-compatibility renderer for any InvoiceDesign row still in the
pre-cutover (`zone_1`/`zone_2`) shape after the one-time production
migration (a row is left in this shape only when the migration mapper
itself couldn't safely convert it — see
`migrate_invoice_designs_to_production_schema`'s own output). Originally
closed PDF-001 (19 August 2026 production audit) — the real
design_data-to-HTML renderer plus its wiring into
pdf_generator.py's render_invoice_pdf/render_invoice_portal_html
via the should_render_dynamic_design branch. Prior to that fix, a saved
...
from apps.invoices.legacy_design_renderer import design_has_real_custom_data, should_render_dynamic_design

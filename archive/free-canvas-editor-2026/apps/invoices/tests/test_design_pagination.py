...
class RendererPaginationTests(InvoicesAPITestCase):
    ...
    def setUp(self):
        ...
        # Real data: URI (no network dependency) — required as of the
        # signature-block-collapses-when-unset fix (design_renderer.py's
        # _element_has_real_content): test_pinned_elements_are_never_
        # duplicated_or_lost_when_flow_content_overflows below asserts the
        # signature label appears exactly once, which needs a real
        # signature_url set to render at all now.
        self.user.profile.signature_url = (

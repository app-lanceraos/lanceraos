...
**GrapesJS re-evaluation (flagged, not decided — §35):** across this project's history, GrapesJS has required at least 4 distinct internal-bug workarounds (broken resize-commit path, SelectorManager class-vs-ID collision, resize/drag view/model desync, no zoom-awareness in resize handles). Each was fixed correctly, but the pattern itself is a signal. **Update (29 August 2026, resize-handle/zoom desync fix — see DECISIONS.md's own entry for the full investigation):** the "no zoom-awareness in resize handles" item on this list is now closed, and — unlike the other 3, which needed patching genuinely undocumented internals (`updateTarget`, a direct `Resizer.prototype` monkeypatch to diagnose the commit bug, a manual `view.render()` resync) — this one had a real, first-class, DOCUMENTED extension point already built for exactly this purpose (`ResizerOptions.mousePosFetcher`, `grapesjs/dist/index.d.ts`). A native `Canvas.setZoom()` alternative was prototyped and rejected first — real, live-tested evidence showed it corrupts a plain resize (left/top shift when they must not, confirmed not caused by canvas panning) — so this is not "GrapesJS quietly does the right thing everywhere once you call the right method"; it required knowing which of two zoom mechanisms to use and which to avoid. Net effect on this build-vs-keep signal: still worth weighing (3 real internal-bug workarounds remain, one of them — the resize/drag view/model desync's own `window.__v2ResyncView` — still load-bearing), but slightly less alarming than "4 undocumented workarounds," since GrapesJS's own public API surface did anticipate this specific need. A related, separate discovery from the same investigation, NOT a GrapesJS bug but an app-level one: `serialization.js`'s `elementComponent()` had been setting `resizable: !el.locked` as a plain boolean on every element instance (added later, for the Layers panel's lock/hide toggle) — since an explicit instance-level Backbone attribute always shadows a component TYPE's own `defaults`, this silently discarded `componentTypes.js`'s entire rich `resizableConfig()` (`silentFrames`, `updateTarget`, and now `mousePosFetcher`) for every real element, unnoticed until this pass — see DECISIONS.md for the full before/after and two newly-surfaced follow-up items (undo not reverting a resize; the resize bounds-clamp not reliably holding under a large, boundary-crossing drag) this same fix made reachable for the first time and did not have time to fully resolve. The final architecture still does **not** mandate replacing GrapesJS — the schema is now fully stable, which is exactly the condition under which a smaller, purpose-built canvas (a plain SVG/DOM drag-resize layer, since the actual interaction surface needed — free 2D positioning, 8-handle resize, no rich text editing, no nested component trees — is narrow) becomes a realistic, lower-risk alternative. This is a genuine build-vs-keep decision requiring product sign-off, not something to decide unilaterally in this document.

**Second update (29 August 2026, same day — drag reposition bug, a real user report that contradicted this document's own first update above; see DECISIONS.md's own second entry for the full investigation):** the first update's implicit claim that drag needed no fix was wrong, and is retracted here explicitly rather than left standing. Root cause, confirmed via live, repeated-trial testing (both the reported CSS-transition-timing hypothesis and an initial hypothesis of my own — a race against GrapesJS's own async commit — were tested directly and definitively falsified, not assumed): a genuine app-level bug, not a GrapesJS one — `DesignEditor.jsx`'s drag-commit mouseup handler tried to identify which component was dragged by walking up from `e.target` looking for a `data-el-type` ancestor, but a real drag's own mouseup lands on GrapesJS's own `.gjs-hovered` highlight overlay (a sibling, never a descendant of the dragged element), so the walk-up found nothing and the entire clamp/resync block silently never ran for a real drag commit — only later, on whatever interaction next happened to land on the element's own content. Fixed by falling back to `ed.getSelected()`, reliable since GrapesJS's own Sorter keeps the dragged component selected throughout. This adds a 5th item to this section's own running tally of GrapesJS-adjacent bugs this project has had to work around — though, same distinction as the mousePosFetcher item above, this one is genuinely an app-level mistake in how this codebase reads DOM events around GrapesJS's own canvas, not a GrapesJS defect itself.

A separate, real, **NOT fixed** finding from the same investigation, more consequential for this section's own build-vs-keep question than either bug above: real browser zoom (Ctrl+/-, pinch-zoom — distinct from this app's own CSS-transform zoom buttons) breaks reliable canvas interaction at a structural level. Confirmed via CDP's `Emulation.setDeviceMetricsOverride` `scale` parameter (the one mechanism, of several tried, confirmed to actually reproduce real zoom's effect on the effective CSS-pixel viewport — `window.innerWidth` genuinely changes). At 90%, 110%, and 125% simulated real browser zoom, selecting the SAME test element that works perfectly at 100% instead hits GrapesJS's own hover-highlight overlay intercepting the click — a different specific intercepting element at each level tested, meaning real zoom shifts rendered canvas content enough for a real mouse click to land on entirely the wrong thing. Not a test-automation artifact — this reflects genuine DOM hit-testing, which would affect a real mouse user identically. Root cause not yet isolated (plausibly an interaction between this app's own outer flex/canvas layout reflowing at the wider effective viewport real zoom produces, and GrapesJS's own highlight-overlay positioning) — flagged here as a real, open, unresolved item rather than force-fixed under this pass's own time budget. A second, smaller, also-unresolved real-browser-zoom-specific bug: at 90% specifically, the editor's own toolbar overlaps (the Redo button intercepts clicks meant for the 100%-zoom button), a genuine responsive-layout gap at that combination, unrelated to the canvas hit-testing issue. Both are a materially different, and arguably more serious, class of finding than anything on this section's own "GrapesJS internal bugs" list — they suggest the interaction between a REAL browser-level zoom feature and this app's OWN separate CSS-transform-based zoom mechanism has never been exercised or verified before this pass, at any point in this project's history, and is not simply another GrapesJS quirk to patch around.

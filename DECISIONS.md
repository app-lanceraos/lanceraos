...
**14 September 2026 — Template Gallery Polish Pass: "Generated by" label, Modern's sidebar-bleed
root cause fixed, brandname auto-sizing, toggle repositioned.**

A follow-up fix pass against the 3 existing templates, triggered by live review of the actual
gallery UI and a real generated Modern PDF — no new templates, no preview-mechanism rework beyond
regenerating the existing static PNGs.

**Part 0 spike — both sub-questions resolved, one real quirk found.**

- **0a (mixed string+url() content):** confirmed WeasyPrint renders `content: "text" url(...)`
  correctly as adjacent inline content — but a REAL, confirmed quirk: an ordinary space character
  (including `white-space: pre`, multiple spaces, and even `\00A0` non-breaking space) immediately
  before a `url()` sibling in the same content list collapses to a measured ~0pt gap in every
  variant tried. Only a genuine fixed-width space character survives: `\2002` (EN SPACE) measured a
  real ~1pt gap, proportionate to the 6.5pt text; `\2003` (EM SPACE) measured ~4.2pt, visibly too
  wide. EN SPACE is what's actually used. This is not a documented WeasyPrint limitation anywhere
  public as far as this investigation found — recorded here as a real, reproducible finding.
- **0b (cross-template collision with the widened right box):** real multi-page renders of all 3
  templates with the new "Generated by" + wordmark content found **zero collisions and needed zero
  new width-safety rules anywhere** — including Modern, once its sidebar-bleed root cause (below)
  was fixed. This is a genuine negative result, not an oversight: Professional and Minimal already
  had ~107–158pt of real clear space between `@bottom-center` and `@bottom-right` with the old bare
  wordmark; the widened content consumes ~47pt more and still clears with margin to spare (real
  measured gaps: 157.9pt for Professional, 106.9pt for Minimal). Modern, once its own
  sidebar-specific override block is removed entirely, renders byte-identically to Professional
  (confirmed: both produce `left-x = 0.0` for the footer's business-name line) — so the
  width-safety mechanism was never promoted into the shared partial, because none of the 3
  templates needed one for this content. `_partials/README.md` is unchanged; the "shared-partials
  convention" is unaffected since nothing was actually shared here beyond what already was.

**Part 1 — branding toggle repositioned.** `TemplateGallery.jsx`'s toggle moved from a full-width
card below the grid to a compact one-line row above it (label + inline switch, one short em-dash
clause instead of a paragraph). Behavior is completely unchanged — same `/auth/profile/` PUT, no
optimistic flip, a real inline error on failure — confirmed by the pre-existing toggle test suite
passing unmodified plus one new positional test (`compareDocumentPosition` confirms it renders
before the grid in source order).

**Part 2 — "Generated by" label added.** This reverses part of the 30 August 2026 decision (this
document, "Generated via LanceraOS text removed from all 3 (superseded by the wordmark)") that
removed a bare text string as redundant once the wordmark existed on its own. Live review found
the bare mark alone was ambiguous — a client could plausibly read the unlabeled wordmark as the
freelancer's OWN logo/branding, not a platform attribution. The correction is deliberately NOT a
reversion to the old bare-text-alone version: the new text is paired directly with the wordmark
("Generated by [wordmark]"), using the EN SPACE separator from the Part 0a finding, `IBM Plex Mono`
at 6.5pt to match the rest of the (now-lighter) footer, and each template's own existing muted
`footer_text_color` — implemented once in `_partials/footer.html`, inherited by all 3 templates
with zero per-template override needed (confirmed by Part 0b).

**Part 3 — Modern's sidebar-bleed root cause fixed, not worked around.** Reading `.sidebar`'s own
rule directly (not assumed) confirmed the actual mechanism: `bottom: -16mm` deliberately bled the
sidebar's dark background PAST the content box, all the way to the true page edge, through the
entire reserved 16mm footer margin — and `@bottom-left`'s 58mm inset existed ONLY to keep footer
text clear of that bled color. Fixed at the root: `.sidebar`'s `bottom` is now unconditionally `0`
(the render-time conditional the Foundation pass added — `0` vs `-16mm` depending on
`single_page_layout`/`wordmark_data_uri` — is gone entirely, a real simplification, not just a fix,
since the whole reason for that conditional no longer exists), which stops the dark fill at the
CONTENT BOX's own bottom edge instead of the true page edge. The entire post-include override
block (`@bottom-left { margin: 0 0 5mm 58mm; width: 50mm; }` and the `@bottom-center`/
`@bottom-right` width/margin siblings it necessitated) is removed — confirmed unnecessary, not
just simplified, by Part 0b's own real render showing byte-identical footer positioning to
Professional with the override block fully absent. **Real, stated visual tradeoff, confirmed with
Ali:** the sidebar's dark background no longer bleeds into the bottom ~16mm strip at the page's
bottom-left corner — that strip is now plain page background on all 3 templates identically
(previously true only for Professional/Minimal, which never had a sidebar to bleed in the first
place). The sidebar still runs full-height through the entire content area above that strip.
Verified via real rendered-page inspection (not the CSS source): the sidebar's own dark fill
rectangle ends at exactly `page_height − 16mm` (measured `796.5354pt` against a computed exact
boundary of `796.5354pt` — a `0.00004pt` difference, floating-point rounding only), never past it;
Modern's footer text starts at real `x=0.0`, identical to Professional's own `x=0.0` (Minimal is
NOT compared here — its own distinct, legitimate 18mm side `@page` margin, unrelated to this pass
or the sidebar fix, correctly produces a different left-x, and asserting otherwise would be
asserting the wrong thing). The full footer matrix and the pre-existing page-count-parity tests
were re-run and stayed green — nothing about page-count logic changed, confirmed rather than
assumed given this touches the exact CSS region that logic depends on.

**Part 4 — sidebar business-name auto-sizing.** `pdf_generator._brandname_font_size_pt`, a real
calibrated linear scale (14pt down to a 10pt floor) based on an isolated WeasyPrint+PyMuPDF sweep
against the REAL Space Grotesk font at the sidebar's real ~30mm effective content width (not
guessed): `"Horizon"` (7 chars) already fits at 14pt; `"Horizon Studio"` (14 chars, the exact real
fixture used throughout this project's own footer/page-count spikes) needed ≤12pt to stop wrapping
to 2 lines; `"Callahan Design Co"` (18 chars) needed 9pt; nothing tested wraps to 1 line above 32
chars at any size down to 9pt. The scale's two calibration points (8 chars → 14pt, 20 chars → 10pt
floor) are chosen so the formula lands on exactly 12.0pt at 14 chars — matching the real
`"Horizon Studio"` measurement precisely, not approximately. Below the floor, a name wraps across
real multiple lines (verified: an extreme 63-character name wraps to 5 real lines at the 10pt
floor, with every real word surviving intact — never truncated, never overflowing into a spurious
2nd page for an otherwise-1-page invoice). 10pt sits comfortably above every other real sidebar
text size (`.contact` 7.5pt, `.pay-block .lbl` 7pt, `.pay-block .link` 6.5pt), so legibility is not
a concern at the floor.

**Part 5 — stale preview PNGs regenerated.** All 3 (`generate_template_previews.py`) re-rendered
through the real, now-fixed pipeline; file hashes confirmed to differ from the pre-pass versions
(not just "the command ran cleanly") — `professional.png` `189535bb…` → `602e3e29…`, `minimal.png`
`8b164240…` → `2dda3c60…`, `modern.png` `925b448b…` → `d8ac0205…`. Visual spot-check of the
regenerated Modern preview confirms all 4 fixes together in one real render: the sidebar's dark
fill stops cleanly above a plain bottom-left corner, the footer sits at the same position as
Professional's, "Generated by [wordmark]" reads correctly bottom-right, and "Horizon Studio" sits
on one real line in the sidebar.

**Verification.** Full project backend suite: 945 passing (`manage.py test --keepdb`, one clean
run — up from 936 before this pass; 9 new tests in `test_footer_rule.py`'s new
`GeneratedByLabelTests`/`ModernFooterMatchesTheOtherTwoTests`/`ModernBrandnameSizingTests`
classes), including the pre-existing, UNMODIFIED
`test_multi_page_invoice_renders_through_safe_path_with_all_items_intact` and
`FooterAndSignaturePinningTests`, both still green, and the Foundation pass's own 18
`test_footer_rule.py` tests, also still green. Frontend: 217 passing (up from 216; 1 new
positional test for the toggle in `TemplateGallery.test.jsx`). `npx vite build` clean.

Alternatives considered: reverting fully to the old bare "Generated via LanceraOS" text-only
footer (no wordmark at all) — rejected, since the wordmark itself was never the problem, only its
lack of a label; keeping Modern's old sidebar-bleed behavior and instead widening the 58mm inset
further to accommodate the new "Generated by" text — rejected once reading `.sidebar`'s own rule
made the real root cause obvious, since patching the inset again would be treating the symptom a
third time rather than removing the actual cause; a fixed (non-scaling) smaller `.brandname` font
size for every business name regardless of length — rejected, since it would needlessly shrink
already-short names that fit fine at 14pt today.

---

**15 September 2026 — Footer Refinement, Gallery Navigation, Signature Centering.**

Six fixes from direct review of real rendered output: a shared real left-margin target across all
3 templates, real vertical alignment between "Generated by" and the wordmark, a reduced footer
height, centered signatures, a Template Gallery breadcrumb, and the sidebar Invoices pill staying
active on real sub-pages. Two of the six needed a real isolated spike first (this WeasyPrint
version has now surprised this project on margin-box CSS three separate times); the other four had
confirmed root causes going in.

**Part 0 spike results, all 4 reported before any production edit.**

- **0a (real measured left-edge, current state):** `professional.html`/`modern.html` = 0.00mm,
  `minimal.html` = exactly 18.00mm — confirmed directly against `minimal.html`'s own real `@page`
  source (`margin: 20mm 18mm ...`, the 3-value shorthand's 2nd term is BOTH left and right) that
  this comes purely from `@page`'s own left margin; `_partials/footer.html` declares no
  `margin-left` of its own today, so there's nothing else it could be coming from.
- **0b (does `margin-left` on `@bottom-left` add to `@page`'s own margin, or replace it?):**
  ADDS — confirmed by 4 isolated renders (0mm+16mm=16mm; 18mm+16mm=34mm, not 16mm; 18mm+(-2mm)=16mm,
  a real negative margin-left works cleanly and lands exactly on target). Real chosen target:
  **16mm**, verified to land all 3 templates within 0.0000mm of each other on the real production
  templates after the fix (not just the isolated spike) — `professional.html`/`modern.html` pass
  `footer_left_inset="16mm"` (0+16=16), `minimal.html` passes `footer_left_inset="-2mm"`
  (18-2=16).
- **0c ("Generated by" + wordmark vertical alignment):** `vertical-align` (tried as `middle`,
  `text-top`, `sub`, and a plain length value) has **ZERO measurable effect** on margin-box
  generated content in this WeasyPrint version — every variant produced byte-identical rendered
  output to the unstyled baseline, confirmed by real PyMuPDF measurement, not assumed to work
  because the property "should." Growing the wordmark SVG's own declared `width`/`height` (to add
  blank space around the mark) doesn't reposition the mark within its own box either — it only
  grows the surrounding inline line's height, which moves the sibling TEXT instead (measured
  directly: the image's own drawn bbox stayed byte-identical across 3 different height values,
  while the text shifted further out of alignment each time — the wrong element was moving). The
  mechanism that actually works: shifting the wordmark SVG's own `viewBox` `min-y` (`viewBox="0
  {min_y} 140 21"`, width/height output attributes held constant) — this moves the rendered ink
  within the SAME fixed-size box without touching surrounding line layout at all (confirmed: text
  position stayed byte-identical across every `min_y` tested). Calibrated against the REAL
  production font (IBM Plex Mono, not the sans-serif fallback an early draft of this spike used —
  re-calibrated once the real font gave a measurably different baseline offset, -3.83pt vs.
  -3.68pt) via a real binary search against actual rendered output, landing on `min_y = -8.13` for
  a real measured offset of **0.002pt** — effectively perfect, sub-visual, confirmed identical
  across all 3 templates on the real production render.
- **0d (reduced footer height, page-count parity):** a real measurement (not a round number picked
  first) showed the actual 2-line 6.5pt footer content occupies only 17.22pt (6.08mm) — the old
  16mm margin was giving it ~14.07pt (4.96mm) of padding on BOTH sides (WeasyPrint centers margin-
  box content vertically within the reserved margin by default, confirmed: top-offset and bottom-
  offset from content were within 0.01pt of each other). Candidate **12mm** (leaving ~2.96mm
  padding each side, a real, visible reduction, still comfortable) was swept against the full prior
  parity matrix — all 3 templates × 8 content lengths (3, 8–13, 40 items) — **zero page-count
  regressions**. Re-checked for new arithmetic dependents per the Foundation pass's own convention:
  `professional.html`'s `.page` padding (`16mm 16mm 0 20mm`) and `minimal.html`'s `.sign-row`
  margin-top (`16mm`) and `modern.html`'s `.main` padding (`14mm 16mm 16mm`) all share the literal
  string "16mm" but are genuinely independent, unrelated CSS values (internal content padding /
  document-flow spacing, not the `@page` margin) — confirmed by grep and by reading each rule's own
  purpose, not assumed safe from the numeric coincidence. Modern's own `.sidebar` no longer has ANY
  margin-dependent value at all (fixed unconditionally to `bottom: 0` in the Polish pass), so there
  was nothing left to re-verify there specifically.

**Part 1 — footer fixes applied.** `_partials/footer.html`'s `@bottom-left` gained
`margin-left: {{ footer_left_inset }}`, a new per-template `with`-clause parameter (same mechanism
as `footer_text_color`). All 3 templates' `@page` bottom margin reduced 16mm → 12mm, with every
real dependent min-height value recomputed (`professional.html`'s `.page`: 281mm → 285mm;
`minimal.html`'s `body.body-single`: 261mm → 265mm; `modern.html`'s `.main.main-single`: 281mm →
285mm). `pdf_generator._generate_wordmark_data_uri` gained a `viewbox_min_y` parameter, defaulting
to the calibrated `-8.13` (the wordmark's one real production use is always paired with this
text, so this is the right default rather than a call-site override). Full footer matrix re-run —
zero regressions, `FooterAndSignaturePinningTests` and the 40-item multi-page regression test both
stayed green, unmodified.

**Part 2 — signature centering.** Confirmed root cause exactly as expected:
`.sign-block { text-align: right; }` in all 3 templates. Changed to `text-align: center` in
`professional.html`/`minimal.html`/`modern.html` (Modern's own `margin-left: auto` kept unchanged —
it positions the whole block at the row's right end via flexbox, a separate concern from the
INTERNAL alignment `text-align` controls). Verified via a real cropped render (not just the CSS
source) that the image, the divider line, and the label all land centered together — see this
pass's own report for the screenshot.

**Part 3 — Template Gallery breadcrumb.** `TemplateGallery.jsx` gained a minimal "Invoices >
Template Gallery" breadcrumb above its `<h1>` — plain text plus a real `react-router-dom` `<Link>`,
not a new shared component, since grep confirmed no breadcrumb pattern exists anywhere else in this
app yet (this is the first one; a heavier shared component can be extracted once a second real
consumer exists). `InvoiceAnalytics.jsx` was checked for the identical gap and found to already
have a real, working "back to Invoices" affordance (an icon-only `ArrowLeft` button beside its own
`<h1>`, wired to `navigate('/invoices')`) — left untouched rather than replaced with a breadcrumb
just for stylistic consistency, per this pass's own explicit fallback instruction.

**Part 4 — sidebar Invoices pill, full `NAV_GROUPS` audit.** Confirmed root cause:
`{ to: '/invoices', ..., end: true }` — React Router's `end: true` restricts the active match to
the exact path. Changed to `end: false`. Every other `NAV_GROUPS` entry was checked against the
REAL routes `App.jsx` defines today (not assumed from the nav list itself): `/clients` is
`end: true` correctly on purpose — no `/clients/*` route exists in `App.jsx` (its detail view is a
state-driven panel, not a routed page, same pattern `Invoices.jsx` itself used before Template
Gallery/Analytics became real routed sub-pages) — revisit only if a real `/clients/*` route is ever
added. `/dashboard` is `end: false` already but has no route in `App.jsx` at all yet — moot.
`/payments`, `/expenses`, `/pnl`, `/tax`, `/proposals`, `/contracts`, `/health`,
`/income-certificate`, `/skill-gap` are all `end: true` but NONE has any real route in `App.jsx`
yet (every one of those modules is "Not started" per CLAUDE.md's own build-status table) — moot
today, not a live bug, but flagged directly in a new comment block above `NAV_GROUPS` so whoever
builds each module re-checks its own `end` value against its real sub-page shape at that time
rather than assuming today's value is still correct once it ships.

**Verification.** Full project backend suite: 948 passing (`manage.py test --keepdb`, one clean
run — up from 945; 3 new tests: `WordmarkVerticalAlignmentTests`,
`SignatureCenteringTests` × 2), including the pre-existing, UNMODIFIED
`test_multi_page_invoice_renders_through_safe_path_with_all_items_intact` and
`FooterAndSignaturePinningTests`. One pre-existing test needed a real, expected update, not a
regression: `test_sidebar_dark_fill_stops_at_the_content_box_boundary_not_the_true_page_edge` had
the OLD 16mm margin hardcoded in its own boundary computation — updated to the new real 12mm value
Part 0d/1 establish, the same class of change the test's own docstring already anticipated.
Frontend: 222 passing (up from 216; a new `AppShell.test.jsx` — this component's first-ever test
file, 4 tests proving the real fix through actual `NavLink` matching at `/invoices`,
`/invoices/templates`, `/invoices/analytics`, and a real negative control at `/clients` — plus 2
new `TemplateGallery.test.jsx` tests for the breadcrumb, and a `renderPage()` `MemoryRouter` wrap
now needed by every existing test in that file since the component renders a real `<Link>`).
`npx vite build` clean. Preview PNGs regenerated (all 3 hashes confirmed changed — visual
confirmation of the reduced footer margin/left-inset/wordmark alignment together in one real
render, see this pass's own report).

Alternatives considered: a positive `margin-left` on `minimal.html` (adding to reach some value
>18mm) instead of the real, working negative-margin reduction — rejected as unnecessarily
roundabout once the isolated spike confirmed negative margin-left works cleanly and precisely;
detecting the sign-block's real rendered width dynamically via drawn-rectangle inspection in the
new signature-centering test — tried first, abandoned once a real cropped screenshot confirmed
production output was already correct and the ONLY problem was the detector's own unreliable
heuristic (a WeasyPrint rendering merge made the thin divider border hard to isolate from
neighboring fills); replaced with a test that computes each template's real known geometry
directly from its own CSS instead, which is both more robust and doesn't depend on guessing at
...
**18 September 2026 (Shared-Partials Hygiene pass — footer inset symmetry + a real pay-online
hyperlink).** Two small, independently-diagnosed bugs, each in a shared partial used by most or all
of the 23 templates (22 selectable + the retired legacy `minimal.html`).

**Issue 1 — `footer.html`'s right side had no calibrated inset at all.** The 15 September 2026
Footer Refinement pass gave `@bottom-left` a per-template `footer_left_inset` so every template's
business-info text lands at the same real absolute left position, but `@bottom-right` (the
"Generated by [wordmark]" content) never got the equivalent. Confirmed by real render + PyMuPDF
measurement, not assumed: the real LEFT inset was 16.0mm for 22 of 23 templates and 12.0mm for
`free_compact.html` specifically (its own `footer_left_inset="-2mm"` against an 14mm `@page` left
margin — an existing, pre-this-pass calibration choice, left untouched); the real RIGHT inset with
no override ranged from 23.282mm (every `@page`-right-margin-0 template) up to 45.282mm
(`free_simple.html`'s own 22mm right margin) — a real, visible left/right imbalance in every one of
the 23 templates.

A real isolated spike (`@page{margin:0;@bottom-right{content:"HELLO";margin-right:10mm;}}`)
confirmed `margin-right` ADDS to `@page`'s own right margin exactly the way `margin-left` already
does on the left (measured 9.9998mm shift for a 10mm override — not assumed from the left side's
own behavior). A second, separate real finding along the way: with NO `margin-right` override at
all, `@bottom-right`'s content does not sit flush against the page's own right-margin boundary the
way `@bottom-left`'s does at `margin-left:0` — it carries a real, constant, intrinsic ~23.282mm gap
regardless of content, confirmed identical across every zero-right-margin template. (A negative
`margin-right` in the isolated 0-margin spike pushed the test content fully off-page and invisible —
irrelevant to the real fix here since every real template's natural gap, ~23mm minimum, is far
larger than the ~7-29mm pull-in each one needed, but recorded as a real limit encountered, not
smoothed over.)

Fix: added `footer_right_inset` to `_partials/footer.html`'s `@bottom-right` rule
(`margin-right: {{ footer_right_inset }}`), calibrated per template as `(that template's own real
measured left inset) − (its own real measured, unadjusted right inset)` — grep-confirmed as the
complete list of all 23 templates that include `footer.html`: `professional.html`, `minimal.html`,
`modern.html`, `free_essential.html`, `free_clean.html`, `free_classic.html`, `free_simple.html`,
`free_compact.html`, `free_freelancer.html`, `free_minimal.html`, `free_modern.html`,
`free_professional.html`, `free_business.html`, `pro_executive.html`, `pro_studio.html`,
`pro_signature.html`, `pro_editorial.html`, `pro_grid.html`, `pro_commerce.html`,
`pro_prestige.html`, `pro_atelier.html`, `pro_consulting.html`, `pro_statement.html`. Every value is
negative (`-7.282mm` for the 10 zero-right-margin templates up to `-29.282mm` for
`free_simple.html`), pulling content IN from that large intrinsic gap rather than pushing it out
from a flush edge, the opposite sign pattern from most `footer_left_inset` values.

**Verification, Issue 1.** Real re-render + PyMuPDF measurement of all 23 templates, before and
after: left inset unchanged (16.0mm ×22, 12.0mm for `free_compact`); right inset now matches its own
template's left inset EXACTLY at 3-decimal precision (16.0/16.0 for 22 templates, 12.0/12.0 for
`free_compact`) — achieved tolerance sub-0.001mm, effectively exact. Real before/after screenshots
(`professional.html`, `free_clean.html`, `free_compact.html`) confirm the wordmark visibly shifted
right with zero clipping, zero overlap, page-number position and wordmark vertical alignment
unaffected. Full page-count-parity sweep (1/3/12/40 items, real git-stash-based before/after) across
all 23 templates: identical matrices before and after (e.g. `professional`: 1/1/2/4 both sides;
`free_compact`: 1/1/1/2 both sides) — zero regressions. `test_footer_rule.py` +
`test_pdf_pipeline.py` (57 tests, incl. `ModernFooterMatchesTheOtherTwoTests`,
`SignatureCenteringTests`, `PageCountParityTests`) — all green.

**Issue 2 — the "Scan to pay" block was never a real hyperlink.** Confirmed directly: neither
`_partials/pay_online.html` nor any of its 3 hand-rolled equivalents (`modern.html`'s
`.pay-block`, `pro_signature.html`'s `.pay-online`, `pro_statement.html`'s `.stm-rail-bottom`)
contained an `<a>` anywhere — all 4 were plain `<div>`s. `pro_statement.html` is a real correction
to this task's own starting premise, which named only `modern.html`/`pro_signature.html` as the
hand-rolled exceptions — a direct grep of `_partials/pay_online.html` usage found 20 real
consumers, confirming `pro_statement.html` was never wired to the shared partial either (its own
`.stm-rail-bottom` markup pre-dates the partial, per the Part 5 entry below).

Fix: `_partials/pay_online.html`'s outer `<div class="pay-online">` became
`<a class="pay-online" href="{{ invoice.payment_page_url }}" style="{{ wrapper_style }}
text-decoration:none; color:inherit;">` — the same URL `qr_code_data_uri` is already built from
(`pdf_generator.build_pdf_context`), confirmed identical, never a second derived value. The
`text-decoration`/`color` reset is an INLINE style directly on the `<a>`, matching STANDARDS.md's
signature-size-ceiling convention (survive every template's own separately-authored CSS with no
per-template override needed). `.pay-online`'s own `display:flex` (declared per-template in
`professional.html`/`minimal.html`, or shared via `base_components.css` for the 20-template-import
consumers) carries over from `<div>` to `<a>` unchanged — confirmed by real render, not assumed
from CSS-selector theory (an anchor is not a flex container by default, but every consuming rule
already declares its own `display`, which wins regardless of the element's default).

The 3 hand-rolled templates got the equivalent local fix: `modern.html`'s whole `.pay-block` became
an `<a href="{{ invoice.payment_page_url }}" style="display:block; text-decoration:none;
color:inherit;">` (needed the explicit `display:block` — `.pay-block` relies on the default block
layout of a `<div>`, which an anchor does not get for free); `pro_signature.html`'s `.pay-online`
div became the equivalent `<a>` (no explicit `display` override needed — `.pay-online` is already
`display:flex` via `base_components.css`); `pro_statement.html`'s `.stm-rail-bottom` became an
`<a>` with the same explicit `display:block`. All 3, like the shared partial, now render nothing at
all when `qr_code_data_uri` is unavailable — a real, deliberate, documented behavior change for
`modern.html`/`pro_signature.html` specifically, which previously showed the "Pay online"/"Scan to
pay" TEXT even without a QR code, only gating the `<img>` itself; aligning them with the shared
partial's own long-standing "no QR, no block" precedent (`pro_statement.html` already gated the
whole block this way before this pass).

**Verification, Issue 2.** Real generated PDFs for all 23 templates, real link-annotation
extraction via PyMuPDF's `page.get_links()`: 23/23 produce a link `uri` exactly matching that
invoice's own real `invoice.payment_page_url` — confirmed programmatically, not by "renders without
error." A real edge-case check (monkeypatched `_generate_qr_data_uri` to return `None` for
`professional.html`/`modern.html`/`pro_signature.html`/`pro_statement.html`, one of each of the 4
distinct markup shapes): all 4 rendered with zero exceptions, zero link annotations, and zero
"Scan to pay" text anywhere in the output — the whole block cleanly disappears, matching the
existing no-QR precedent. Real before/after screenshots (`professional.html`, `modern.html`, full
page, not just the footer band) are pixel-identical apart from Issue 1's own wordmark shift — no
stray underline, no color shift, no layout change from the link conversion itself. Full page-
count-parity sweep, same 1/3/12/40-item matrix, re-run against the FINAL state (both fixes
combined) versus the true original baseline: identical across all 23 templates — zero regressions
from either fix, together.

**What didn't work as hypothesized, stated plainly.** The task's own starting premise that only
`modern.html` and `pro_signature.html` hand-roll the pay-online block was wrong by one —
`pro_statement.html` does too; caught by grepping the partial's real usage rather than trusting the
prompt's illustrative list. A negative `margin-right` was expected to behave symmetrically to a
negative `margin-left` and did, in the sense that it adds signed offset the same way — but the
isolated 0-margin spike's own negative case pushed test content off-page entirely, a real
WeasyPrint clipping behavior worth recording even though it turned out not to matter for any real
template's own (much larger) intrinsic gap.

---

**18 September 2026 (correction to this same day's earlier "Shared-Partials Hygiene pass" entry
above — the wordmark was clipping off the true page edge).** That entry's own verification claimed
the right inset matched the left "to sub-0.001mm" across all 23 templates with "zero clipping." It
was wrong. Confirmed directly against a real render of `professional.html`, not assumed: the
"Generated by " TEXT did land exactly on target (right edge at 549.92pt = 16.00mm from the true
right edge — correct). The actual wordmark — the real vector "LanceraOS" mark, rendered from the
embedded SVG's `<path>`, trailing the text via `content: "Generated by\2002" url(...)` — had its own
real right edge at 599.226pt = 211.394mm on a 210.00mm-wide page: **1.394mm past the true page
edge**, on every page.

**Root cause.** The prior pass's own measurement script checked `page.get_text()` for the text and
`page.get_image_info()` for a possible image, took whichever was further right, and treated that as
"the content's real right edge." Neither call sees the wordmark: it isn't a raster image
(`get_image_info()` only finds embedded raster XObjects) and it isn't text (`get_text()` doesn't see
vector path fills). So every one of that pass's "unadjusted right inset" measurements was silently
measuring the TEXT's own position only, with the wordmark — which sits to the right of the text,
separated by the en-space — invisible to the check entirely. Before that pass, the natural ~23mm
gap (`@bottom-right` with no override) had enough slack for the wordmark's own real width to fit
inside it comfortably regardless of exactly where the text landed. Pulling that gap in to 16mm
(correctly, for the text) ate exactly the slack the wordmark needed and pushed it past the edge.

**Step 0 — proving the measurement method before trusting it.** Built a corrected measurement using
`page.get_drawings()`: filter to drawn paths inside the true footer band (`y0 >= page_height - 25pt`)
whose fill color matches that template's own real `footer_text_color`/`_WORDMARK_FILL_BY_TEMPLATE`
hex (within a small RGB tolerance), and take the union bounding rect of the matches (the wordmark is
several subpaths — 1 matched path per template in practice, but the method doesn't assume that).
Run FIRST against the current, still-broken `professional.html`, before trusting it for anything
else: it reported `wordmark_x1 = 599.226pt`, page width `595.276pt` → **overflow = 1.3936mm** —
matching the independently-reported ~1.394mm exactly. Method proven against the known-broken state
before being used to verify any fix.

**Step 1 — the real scope, all 23 templates, measured directly (not assumed from margin groupings).**
Every one of the 23 templates that include `_partials/footer.html` (reconfirmed by grep — same list
as the prior entry: `professional`, `minimal`, `modern`, `free_essential`, `free_clean`,
`free_classic`, `free_simple`, `free_compact`, `free_freelancer`, `free_minimal`, `free_modern`,
`free_professional`, `free_business`, `pro_executive`, `pro_studio`, `pro_signature`, `pro_editorial`,
`pro_grid`, `pro_commerce`, `pro_prestige`, `pro_atelier`, `pro_consulting`, `pro_statement`) was
clipping. The 22 templates targeting a 16mm inset all measured an IDENTICAL real overflow of
**1.3936mm** (not a coincidence — since those 22 all land the text at the identical absolute 16mm
position by design, and the wordmark follows it by the same fixed en-space + SVG width in every
one, the whole content run sits at an identical absolute box position across all 22, wordmark
included). `free_compact.html` (12mm text target) measured a real overflow of **5.3937mm** — larger,
since its target sits further right, giving the trailing wordmark less room before the true edge.
Confirmed: not Ledger-specific, not limited to the 10 zero-`@page`-right-margin templates as
suspected — every single one of the 23 was clipping, uniformly within its own target group.

**Step 2 — the fix, re-derived properly, not patched by an estimated delta.** For each template,
computed the wordmark's real inset with `footer_right_inset` at zero (`baseline_inset = current
real measured wordmark inset − currently-applied footer_right_inset`, solving the same additive
relationship the prior pass already confirmed empirically — `margin-right` adds directly to
whatever the box's position already is — rather than assuming the prior pass's OWN target numbers
were a valid starting point to nudge). Then `new_footer_right_inset = (target + safety_margin) −
baseline_inset`. This is a full re-derivation from a correct (wordmark-based) measurement, not the
prior per-template values plus a guessed correction — the task's own explicit instruction, since
patching the wrong-element numbers by an estimated offset risked compounding the same class of
error rather than fixing it.

**Safety margin, explicit decision.** Added a flat **+1.0mm** to both targets (16mm → 17mm, 12mm →
13mm) rather than landing exactly on the theoretical target to the sub-millimeter, specifically
because the previous pass's own "exact to sub-0.001mm" claim is what shipped a real 1.4-5.4mm
overflow — a razor-thin, unbuffered target proved fragile once the actual outermost element wasn't
the one being measured. 1mm is large enough to absorb real-world variance this measurement can't
fully rule out (font-hinting/subpixel differences across WeasyPrint or font versions, the SVG's own
sub-pixel width) while staying visually indistinguishable from the original 16mm/12mm intent.

Corrected `footer_right_inset` values (old → new), all 23 templates, grouped by their new value
(the grouping differs from the old entry's own groups, since the fix's target now depends on each
template's real `@page` right margin plus the wordmark's own trailing width, not the text alone):

- `professional`, `modern`, `free_minimal`, `free_modern`, `free_professional`, `free_business`,
  `pro_atelier`, `pro_studio`, `pro_signature`, `pro_consulting`, `pro_prestige`, `pro_statement`:
  `-7.282mm` → **`11.112mm`**
- `minimal`, `free_essential`, `free_freelancer`, `pro_executive`: `-25.282mm` → **`-6.888mm`**
- `free_classic`: `-24.282mm` → **`-5.888mm`**
- `free_compact`: `-25.282mm` → **`-6.888mm`** (own 12mm+1mm target, not the 16mm+1mm group)
- `free_clean`: `-27.282mm` → **`-8.888mm`**
- `pro_commerce`, `pro_grid`: `-21.282mm` → **`-2.888mm`**
- `pro_editorial`: `-23.282mm` → **`-4.888mm`**
- `free_simple`: `-29.282mm` → **`-10.888mm`**

Every new value is LESS negative (or positive) than its old value — the whole content run moves
further left/inward than before, exactly as expected once the trailing wordmark (not the leading
text) is the thing being pulled onto the page. The text itself, as a direct and correct consequence,
now sits further from the right edge than 16mm/12mm (its own real new position wasn't separately
targeted — it's whatever the wordmark's own correct position leaves it at).

**Verification.**
- Real re-measurement (the Step-0-proven `get_drawings()` method) of all 23 templates after the
  fix: every one of the 22 "16mm-group" templates now measures the wordmark's real right edge at
  **17.0004mm** from the true edge (target 16mm + 1mm margin); `free_compact.html` measures
  **13.0003mm** (target 12mm + 1mm). Zero overflow anywhere — real margin to spare, not "closer than
  before."
- Real high-DPI (6x zoom, true bottom-right 45mm×20mm corner, no extended/artificial crop)
  before/after screenshots for 5 templates spanning different magnitudes — `professional.html`
  (`+11.112mm`, was `-7.282mm`), `free_compact.html` (`-6.888mm`, its own 12mm target),
  `free_simple.html` (`-10.888mm`, the largest swing), `pro_grid.html` (`-2.888mm`), `minimal.html`
  (`-6.888mm`, was `-25.282mm`, the biggest raw before/after delta): every "before" shot shows the
  trailing "S" of "LanceraOS" visibly cut off at the crop's right edge (the true page edge); every
  "after" shot shows the complete, unclipped wordmark with visible white margin before that same
  edge.
- Left side re-confirmed untouched: real `get_text()` measurement of `@bottom-left`'s business-info
  text across all 23 templates — unchanged at exactly **16.0mm** (22 templates) / **12.0mm**
  (`free_compact`), byte-identical to both the pre-fix and the original 15 September baseline. This
  fix touched `footer_right_inset` only.
- Full page-count-parity sweep (1/3/12/40 items × all 23 templates): identical to the prior entry's
  own post-fix matrix (e.g. `professional`: 1/1/2/4; `free_compact`: 1/1/1/2) — zero regressions from
  this correction.
- Full `apps.invoices` test suite: 817 tests, all passing (matching the prior entry's own count) —
  no test asserts the wordmark's exact pixel position, so none needed updating, but the full suite
  re-run confirms no other regression.

**What didn't work as hypothesized, stated plainly.** The prior pass's own verification method
(`get_text()` + `get_image_info()`) looked reasonable and produced numbers that were internally
consistent (they matched `@page` margin arithmetic exactly) — consistency was mistaken for
correctness. A method can be perfectly self-consistent and still be measuring the wrong element
entirely; the only thing that would have caught this before shipping is exactly what Step 0 does
here — proving the method detects a KNOWN bad state before trusting it to confirm a good one. That

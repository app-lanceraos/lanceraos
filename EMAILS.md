...
| Reset your password | `send_password_reset_email` | **Two trigger paths, one mechanism** (`auth.send_password_reset_link`): (1) anonymous `POST /auth/forgot-password/` for an existing, verified, non-OAuth account; (2) authenticated `POST /auth/security/password-reset/request/` — Settings > Security's "Email Me a Reset Link", for someone signed in who doesn't know their current password (rejects OAuth-only and unverified accounts; 3/hour per user). Both use the same token generator and this same email, and both are completed at `POST /auth/reset-password/<uid>/<token>/` | **Differs by path, on purpose.** (1) **Yes**, via `send_password_reset_email_task` (`synchronous=False`) — required so the anonymous endpoint's response time can't reveal which emails are registered. (2) **No** — sent inline (`synchronous=True`), like the other authenticated-action emails below; a failed send returns a real 503, never a false "check your email" (fixed 21 September 2026 — it had inherited (1)'s `.delay()` and reported success with no Celery worker running) | `password_reset_request` (both paths; audit `metadata.trigger` is `forgot_password` or `settings_security`; path (2) logs it only when the send succeeded) | No |
...
| Add a password to your LanceraOS account | `send_add_password_confirmation_email` | `POST /auth/security/add-password/request/` — OAuth-only account requesting to add a password | No | `add_password_requested` (`add_password_completed` is also logged on success, but has no email of its own — completing the flow signs the person out via the `pca` mechanism, so the in-app success screen carries the confirmation instead) | No — neither event is on the notification allowlist |
| Your verification code | `send_2fa_code_email` | Every login needing a 2FA code, and every explicit "resend code"; also reused as-is by admin panel login (`apps/admin_panel/views.py`), which requires 2FA unconditionally | No | `login_2fa_required` (main app) / `admin_login_2fa_required` (admin panel — distinct event name, same email function) | No |

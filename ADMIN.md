...
| Invoices + Clients | `apps/clients/` backend is real now (Client/ClientNote/ClientTag, full CRUD + archive/restore/flag/notes/tags/analytics), but this module's admin screen is deliberately **not yet built** — per the admin panel's own sequencing decision (see `DECISIONS.md`), an admin screen is built once there's something real to administer, as its own step (step 20 of this module's build order in `INVOICES_CLIENTS_TECHNICAL_SPEC.md`), not alongside the app's first backend prompt. `apps/invoices/` itself doesn't exist yet at all. | Backend (partial) built; admin screen not started |

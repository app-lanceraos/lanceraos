...
import { ACCENT, ERROR, publicBtnPrimary, publicInputStyle } from './portalShared'
...
export default function PortalRequestLinkForm() {
  ...
  if (done) {
    return (
      <p style={{ margin: '16px 0 0', fontSize: '0.85rem', color: ACCENT }}>
  ...
  return (
    <form onSubmit={handleSubmit} style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      />
      {error && (
        <p style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.78rem', color: ERROR }}>{error}</p>
      )}
      <button type="submit" disabled={busy} style={{ ...publicBtnPrimary, width: '100%', opacity: busy ? 0.5 : 1, cursor: busy ? 'not-allowed' : 'pointer' }}>

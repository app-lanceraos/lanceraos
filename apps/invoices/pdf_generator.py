...
TEMPLATE_MAP = {
    ...
    # 20-Template Import, Batch 3 (14 September 2026).
    'free_minimal': 'invoices/free_minimal.html',
...
DEFAULT_TEMPLATE_COLORS = {
    ...
    # 20-Template Import, Batch 3 (14 September 2026) — free_minimal's
    # prototype also sets --accent equal to --ink (same monochrome
    # convention as Compact).
    'free_minimal': ('#17181b', '#17181b'),
...
# Footer Refinement pass (15 September 2026) — the real, calibrated
# vertical shift (in SVG viewBox user units, NOT pt) that aligns the
# wordmark's own drawn ink with the vertical center of the "Generated by"
# text beside it, at that text's real production font/size (IBM Plex
# Mono, 6.5pt). Confirmed via an isolated spike this pass ran first:
# CSS `vertical-align` (tried as middle/text-top/sub/a plain length) has
# ZERO measurable effect on margin-box generated content in this
# WeasyPrint version — every variant produced the exact same rendered
# position, confirmed by real PyMuPDF measurement, not assumed to work
# because the property "should". Growing the SVG's own declared
# width/height (to add blank space around the mark) doesn't reposition
# the mark within its own box either — it only changes the SURROUNDING
# LINE's height, which moves the sibling TEXT instead (measured: the
# text shifted, the image's own drawn bbox didn't move at all). The
# mechanism that actually works: shifting the viewBox's own `min-y`
# (`viewBox="0 {min_y} 140 21"`) — negative moves the rendered ink DOWN
# within the SAME fixed-size box (confirmed: text position stayed
# byte-identical across every min_y value tested, since the box's own
# width/height attributes — and therefore surrounding line layout —
# never change). Calibrated by a real binary search against actual
# rendered output (not computed from font metrics) to a real measured
# offset of 0.002pt at min_y=-8.13 — effectively perfect, sub-visual.
_WORDMARK_FOOTER_VIEWBOX_MIN_Y = -8.13


def _generate_wordmark_data_uri(fill_color, width_px=_WORDMARK_FOOTER_WIDTH_PX, viewbox_min_y=_WORDMARK_FOOTER_VIEWBOX_MIN_Y):
    """
    ...
    `viewbox_min_y` (Footer Refinement pass, 15 September 2026) — see
    `_WORDMARK_FOOTER_VIEWBOX_MIN_Y`'s own comment for the full mechanism
    and calibration. Default is tuned for this wordmark's one real
    production use (paired with "Generated by" text in the footer); a
    future caller rendering the mark alone, with no adjacent text to
    align to, should pass `viewbox_min_y=0`.
    """
    height_px = width_px * 21 / 140
    svg = (
        f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 {viewbox_min_y} 140 21" '

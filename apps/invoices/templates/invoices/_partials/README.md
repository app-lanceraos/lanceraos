...
(Shared-Partials Hygiene pass, 18 September 2026) does the same for
`@bottom-right`'s "Generated by [wordmark]" content on the right. Both
are real negative-or-positive mm values, derived per template from a
real render + measurement, never from `@page` margin arithmetic alone —
see `footer.html`'s own two dated comments and DECISIONS.md's matching
entries for the full per-template values and the real WeasyPrint margin-
box quirks each one uncovered (`margin-left`/`margin-right` both ADD to
`@page`'s own margin rather than replacing it; `@bottom-right` with no
override carries a real, constant ~23.282mm intrinsic gap from the page's
own right-margin boundary that `@bottom-left` does not have on the left).

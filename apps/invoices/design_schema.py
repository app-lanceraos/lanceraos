...
from apps.invoices.legacy_design_schema import OVERLAP_EPSILON_MM
...
# Phase 1 (rotation/ellipse/footer/crop schema extensions) — 'ellipse'
# added alongside the original 4. Renders via the exact same fill/border
# resolution `rectangle` already uses (design_renderer.attach_generic_
...
# Phase 1 — a direct Python port of the editor's own real rotation math
# (invoice-editor/src/utils/geometry.js's rotateVector/rotatedBoundingBox),
# not a looser reinvention: the same "rotate each of the 4 corners around
# the box's own center, then take the axis-aligned min/max of the rotated
# corners" algorithm, so a design saved by the editor and validated here
# agree on exactly what "overlaps"/"exceeds the page" means for a rotated
# element. For rotation=0 (every element that predates this field, and
# every element that never sets it), this returns EXACTLY {minX:x,
# maxX:x+width, minY:y, maxY:y+height} — byte-identical to the plain
# unrotated box every existing check already used — so this is a strict,
# backward-compatible generalization, never a behavior change for
# anything that doesn't use rotation.
def _rotate_vector(dx, dy, degrees):
...
def rotated_bounding_box(element):
    """
    The axis-aligned box that encloses a (possibly rotated) element —
    what every overlap/bounds check below actually tests against, never
    the raw x/y/width/height directly, since two rotated elements can
    visually overlap well before their unrotated boxes would (and,
    symmetrically, a rotated element's TRUE footprint can exceed its own
    unrotated box). Rotation pivots around the element's own center,
    matching the editor's identical convention exactly.
    """
    x, y, width, height = element['x'], element['y'], element['width'], element['height']
    rotation = element.get('rotation') or 0
    # A non-numeric rotation is already reported as its own schema error
    # by _validate_element — the overlap/bounds passes below still run
    # against every structurally-valid-geometry element regardless (see
    # _validate_element_list), so this must degrade to "treat as
    # unrotated" rather than crash a completely separate check on a value
    # that's already flagged invalid elsewhere.
    ...
    rotated = [_rotate_vector(lx, ly, rotation) for lx, ly in corners]
    xs = [cx + rx for rx, _ in rotated]
    ys = [cy + ry for _, ry in rotated]
...
# footer already shows (business identity left, a genuine multi-page-only
# "Page X of N" counter center, "Generated by" + the LanceraOS wordmark
# right — see design_renderer.py's own footer rendering and
# pdf_generator.py's real, existing @bottom-left/@bottom-center/
# @bottom-right mechanism this reuses) — this schema object exists to
# turn that footer ON for a v2 design and customize its STYLE, never to
# let a design author rebind what each slot shows. Optional; absent means
# no footer at all, exactly today's behavior for every existing design —
# `{}` (every key omitted) is valid and means "show the default-styled
# footer". `show_wordmark` is the forward-compatible premium-gating flag
# named in the phase spec — combined at render time with the one real,
# existing `_is_premium_branding_enabled` hook (pdf_generator.py), not a
# second, parallel "is this premium" concept.
FOOTER_STYLE_STRING_KEYS = ('text_color', 'background_color', 'divider_color', 'font_family')


def _validate_footer(footer, errors):
    if not isinstance(footer, dict):
        errors.append('page.footer, if present, must be an object.')
        return
    style = footer.get('style')
    if style is None:
        return
    if not isinstance(style, dict):
        errors.append('page.footer.style, if present, must be an object.')
        return
    for key in FOOTER_STYLE_STRING_KEYS:
        value = style.get(key)
...
def _validate_element(element, label, errors, *, allowed_types):
    ...
    # Phase 1 — rotation (degrees, float; absent means 0, so every
    # existing element stays valid with no migration). Restricted to
    # 'pinned' elements (the default, absent means pinned too — see
    # LAYOUT_MODES' own docstring): a 'flow' element's real footprint at
    # render time is content-driven and unknown at design/validation
    # time, so a rotated one would break _group_into_render_chains' own
    # anchor math (design_renderer.py) in a way this schema can't verify
    # is safe. Rejected here rather than silently ignored at render time.
    if 'rotation' in element:
        rotation = element['rotation']
        if not _is_number(rotation):
            errors.append(f'{label}.rotation must be a number (degrees), if present.')
        elif rotation != 0 and element.get('layout_mode') == 'flow':
            errors.append(f'{label}.rotation must be 0 for a flow-layout element — rotation is only valid on pinned elements.')

    # Phase 1 — non-destructive image crop: a rectangle, in fractions
    # (0-1) of the SOURCE image's own dimensions, of which region to show.
    # Only meaningful on a generic `image` element; stored as a sibling
    # of the element's geometry (never re-encoding/replacing the image
    # itself — src stays the original asset, see design_renderer.py's
    # attach_generic_content for how this renders). Optional; absent
    # means the full, uncropped image — today's only behavior, unchanged.
    if 'crop' in element:
        crop = element['crop']
        if element.get('type') != 'image':
            errors.append(f'{label}.crop is only valid on a generic "image" element.')
        elif not isinstance(crop, dict):
            errors.append(f'{label}.crop, if present, must be an object.')
        else:
            missing_crop = {'x', 'y', 'width', 'height'} - crop.keys()
            if missing_crop:
                errors.append(f'{label}.crop is missing required key(s): {", ".join(sorted(missing_crop))}.')
            else:
                for field in ('x', 'y', 'width', 'height'):
                    if not _is_number(crop[field]) or not (0 <= crop[field] <= 1):
                        errors.append(f'{label}.crop.{field} must be a number between 0 and 1.')
                if _is_number(crop.get('width')) and _is_number(crop.get('x')) and crop['width'] <= 0:
                    errors.append(f'{label}.crop.width must be greater than zero.')
                if _is_number(crop.get('height')) and _is_number(crop.get('y')) and crop['height'] <= 0:
                    errors.append(f'{label}.crop.height must be greater than zero.')
...
def _boxes_overlap_rotated(a, b):
    """
    Phase 1 — the rotation-aware replacement for legacy_design_schema's
    plain axis-aligned boxes_overlap: compares each element's REAL
    footprint (rotated_bounding_box above) rather than its raw x/y/width/
    height, using the exact same OVERLAP_EPSILON_MM tolerance and
    edge-touching semantics boxes_overlap already established (kept
    identical on purpose — this is a generalization of that function, not
    a different policy). For two rotation=0 elements, rotated_bounding_box
    returns their plain unrotated box, so this is byte-identical to
    boxes_overlap's own result for every element that predates rotation.
    """
    box_a = rotated_bounding_box(a)
    box_b = rotated_bounding_box(b)
    return not (
        box_a['maxX'] <= box_b['minX'] + OVERLAP_EPSILON_MM
        or box_b['maxX'] <= box_a['minX'] + OVERLAP_EPSILON_MM
        or box_a['maxY'] <= box_b['minY'] + OVERLAP_EPSILON_MM
        or box_b['maxY'] <= box_a['minY'] + OVERLAP_EPSILON_MM
    )
...
def _validate_overlap(all_valid_elements, errors):
    ...
    Phase 1 — rotation-aware (see _boxes_overlap_rotated above): an
    axis-aligned test would pass designs that visibly overlap once
    rotation exists (two rotated boxes whose unrotated footprints happen
    not to touch) and fail designs that don't (a rotated box's own
...
def _validate_page_bounds(all_valid_elements, page, errors):
    ...
    for list_name, i, element in all_valid_elements:
        ...
        # does: a rotated element pivots around its own center, so its true
        # left/top edge can sit outside its own unrotated box even when
        # x/y themselves look safely on-page. Byte-identical to the plain
        # x/x+width check below for rotation=0 (every existing element).
        box = rotated_bounding_box(element)
        label = f'{list_name}.elements[{i}] ({element.get("type")})'

        if box['minX'] < -OVERLAP_EPSILON_MM:
            errors.append(f'{label} has x={element["x"]} — an element may not start before the left edge of its coordinate space (x >= 0).')
        if box['minY'] < -OVERLAP_EPSILON_MM:
            errors.append(f'{label} has y={element["y"]} — an element may not start before the top edge of the page (y >= 0).')
        if box['maxX'] > bound_w + OVERLAP_EPSILON_MM:
            errors.append(
                f'{label} extends beyond the right edge of its coordinate space '
                f'(x={element["x"]} + width={element["width"]} = {box["maxX"]}, which exceeds {bound_w}).'
            )
